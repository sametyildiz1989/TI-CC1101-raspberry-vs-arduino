...
#define PACKET_SIZE 65534

//--------------------------[Global CC1100 variables]--------------------------
uint8_t Tx_fifo[FIFOBUFFER], Rx_fifo[FIFOBUFFER], sendBuffer[FIFOBUFFER];
char samet_array[PACKET_SIZE];
uint8_t My_addr, Tx_addr, Rx_addr, Pktlen, Lqi, Rssi;
uint8_t rx_addr,sender,lqi;
 int8_t rssi_dbm;

int cc1100_freq_select, step, numberOfPackets, remainingBytes, fileLength, cc1100_mode_select, cc1100_channel_select;
...
int main(int argc, char *argv[]) {
    ...
    cc1100_freq_select=0x02;//434 MHz
    cc1100_mode_select=0x02;//100kbps
    Rx_addr=3;
    My_addr=3;
    interval=100;//ms interval
    tx_retries=0;//tx retry
    cc1100_channel_select=1;//channel
  ...
  /*	while((opt = getopt(argc, argv, "hVva:r:i:t:c:f:m:")) != -1) {
  		...
  		case 'f':
  			...
  			switch(cc1100_freq_select){
  				case 315:
  					cc1100_freq_select = 0x01;
  					break;
  				case 434:
  					cc1100_freq_select = 0x02;
  					break;
  				case 868:
  			...
  			case 'm':
  				...
  				case 1:
  					cc1100_mode_select = 0x01;
  					break;
  				case 38:
  					cc1100_mode_select = 0x02;
  					break;
  				case 100:
  					cc1100_mode_select = 0x03;
  					break;
  				case 250:
  ...
  */
    ...
    Utility util(fileName);
    fileLength = util.fileToHex(samet_array);
    char *newBuf = (char *)malloc(fileLength);
    memcpy(newBuf,samet_array,fileLength);

    for (int i = 0; i < fileLength; i++) {
      Tx_fifo[i] = (uint8_t)(newBuf[i]);
    }

    Pktlen = 0x10;

    numberOfPackets = ( fileLength /  (int)Pktlen );
    remainingBytes = ( fileLength %  (int)Pktlen );
    for (int j = 0; j < numberOfPackets; j++) {
      step = j * Pktlen;
      sendBuffer[0]=Pktlen;
      sendBuffer[1]=My_addr;
      sendBuffer[2]=Rx_addr;
      for (int i = 0; i < Pktlen; i++) {
        sendBuffer[3+i] = Tx_fifo[step + i];
      }
      uint8_t res = cc1100.sent_packet(My_addr, Rx_addr, sendBuffer, Pktlen+3, tx_retries);

      delay(10);
    }
    step = step + Pktlen;
    uint8_t lastBuffer[remainingBytes+3];
    lastBuffer[0]=Pktlen;
    lastBuffer[1]=My_addr;
    lastBuffer[2]=Rx_addr;
    for (int i = 0; i < remainingBytes; i++) {
      lastBuffer[3+i] = Tx_fifo[step + i];
    }
    uint8_t res = cc1100.sent_packet(My_addr, Rx_addr, lastBuffer, remainingBytes+3, tx_retries);
    printf("**File is Sent!**\n");
